    ...
    for (int i = 0; i < (SECTOR_SIZE / 32); i++) {
      if (memcmp(rawArr + (32 * i), filename, 11) == 0 &&
        ...
        *fatdir = *(pFAT32_Directory)(&rawArr[32 * i]);
        ...
        printf("[search] low=%d low1=%x low2=%x\n", (*fatdir).firstClusterLow,
...
#if UNSAFE_DBG
  printf("[read] filesize=%d cluster=%d\n", dir->filesize,
         dir->firstClusterLow);
#endif
  char *out = *rawOut;
  int   curr = 0;
...
int fileReaderTest() {
  ...
  while (1) {
    ...
    // }
#if UNSAFE_DBG
    printf("[parse] FAT32-compatible filename: %s\n", modifiable);
#endif
...
#if UNSAFE_DBG
    printf("[search res] filename=%s attr=0x%x low=%d\n", dir.filename,
